...
int workbook(int n, int k, int arr_count, int* arr) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int problem = 1; problem <= problems_in_chapter; problem++) {
            // Check if the current problem matches the page number
            if (problem == page_number) {
                special_problems++;
            }

            // Go to the next page after every k problems or the last problem
            if (problem % k == 0 || problem == problems_in_chapter) {
                page_number++;
            }
        }
    }
