...
long taumBday(int b, int w, int bc, int wc, int z) {
    // Calculate the minimum cost for black gifts
    long costBlack = std::min((long)b * bc, (long)b * (wc + z));

    // Calculate the minimum cost for white gifts
    long costWhite = std::min((long)w * wc, (long)w * (bc + z));
